...
-- Migrate data from settings table to user_learning_preferences table (if settings table exists)
-- Check if settings table exists before attempting migration
DO $$
BEGIN
    IF EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'settings') THEN
        ...
        RAISE NOTICE 'Settings table found and data migrated successfully';
    ELSE
        RAISE NOTICE 'Settings table does not exist, skipping migration';
    END IF;
END $$;
